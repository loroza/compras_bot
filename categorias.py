...
async def menu_categorias(message: types.Message, state: FSMContext):
...
async def categoria_emoji(message: types.Message, state: FSMContext):
    ...
    if ok:
        await message.answer(
            f"✅ Categoria *{emoji} {nome}* criada!",
            reply_markup=kb_categorias_menu(),
            parse_mode="Markdown",
        )
    else:
        await message.answer(
            "⚠️ Já existe uma categoria com esse nome.",
            reply_markup=kb_categorias_menu(),
        )
...
async def menu_edicao_categoria(message: types.Message, state: FSMContext):
    ...
    if message.text == "🗑️ Excluir":
        ...
        return await message.answer(
            f"Confirma excluir *{cat_nome}*? Digite SIM para confirmar.",
            parse_mode="Markdown",
        )
...
async def categoria_renomear(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Categoria renomeada para *{novo_nome}*.",
        reply_markup=kb_categorias_menu(),
        parse_mode="Markdown",
    )
...
async def categoria_novo_emoji(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Emoji atualizado para *{novo_emoji}*.",
        reply_markup=kb_categorias_menu(),
        parse_mode="Markdown",
    )
...
async def categoria_confirmar_exclusao(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"🗑️ Categoria *{cat_nome}* excluída.",
        reply_markup=kb_categorias_menu(),
        parse_mode="Markdown",
    )