...
flowchart TB
  ...
  ComprasMenu --> VoltarCompras["⬅️ Voltar Compras"]
  ...
  AdicionarItensList --> NavegCatalogoList[Navegar catálogo → adicionar à lista]
  ...
  FinalOpcao -->|Não| VoltarOrigem

  %% Ver Carrinho
  VerCarrinho --> CarrinhoMenu[Menu do Carrinho]
  CarrinhoMenu --> RemoverItem["🗑️ Remover Item"]
  ...
  ListasMenu --> RemoverItemLista["🗑️ Remover Item"]
  ...
  OrcMenu --> EditarOrc["✏️ Editar orçamento"]
  EditarOrc --> IncluirItem["➕ Incluir item"]
  EditarOrc --> ExcluirItem["🗑️ Excluir item"]
  EditarOrc --> EditarItem["✏️ Editar item"]

  %% Histórico (compras)
  Historico --> HistMenu[Histórico de compras]
  HistMenu --> VerDetalhe[Selecionar compra → ver itens / total]

  %% Trocar departamento
  TrocarDept --> EscolherDepartamento

  %% utility
  VoltarOrigem[Voltar ao menu / origem] --> MenuPrincipal
